...
import (
	...
	"github.com/biny-go/eagproto/cmd/eagproto/internal/base"
)
...
var CmdClient = &cobra.Command{
	...
	Long:  "Generate the proto client code. Example: eagproto proto client helloworld.proto",
	Run:   run,
}
...
func init() {
	if protoPath = os.Getenv("EAGPROTO_PROTO_PATH"); protoPath == "" {
...
func run(_ *cobra.Command, args []string) {
	...
	if err = look("protoc-gen-go", "protoc-gen-go-grpc", "protoc-gen-go-http", "protoc-gen-go-errors", "protoc-gen-openapi"); err != nil {
		// update the eagproto plugins
		cmd := exec.Command("eagproto", "upgrade")
...
func generate(proto string, args []string) error {
	...
	inputExt := []string{
		"--proto_path=" + base.EagprotoMod(),
		"--proto_path=" + filepath.Join(base.EagprotoMod(), "third_party"),
		"--proto_path=" + filepath.Join("..", "eagproto"),
		"--proto_path=" + filepath.Join("..", "eagproto", "third_party"),
