...
// EagprotoMod returns eagproto mod.
func EagprotoMod() string {
	...
	// 包版本路径
	if path, err := ModuleVersion("github.com/biny-go/eagproto"); err == nil {
		// $GOPATH/pkg/mod/github.com/biny-go/eagproto
		return filepath.Join(cachePath, path)
	}
	// 指定路径
	// $GOPATH/src/github.com/biny-go/eagproto
	return filepath.Join(gopath, "src", "github.com", "biny-go", "eagproto")
}
